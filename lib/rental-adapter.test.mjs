...
test("rejects invalid rental input", () => {
  ...
  for (const input of invalidInputs) {
    assert.throws(() => validateRentalInput(input, { today: "2026-09-17" }), RentalInputError);
  }
});

test("rejects rental periods that start before today", () => {
  assert.throws(
    () => validateRentalInput({
      itemId: 3,
...
test("rental availability requires available status and positive stock", () => {
  assert.equal(isRentalAvailable({ status: "unavailable", stock: 5 }, 1), false);
  assert.equal(isRentalAvailable({ status: "available", stock: 0 }, 1), false);
});

test("rental availability rejects quantity above stock and accepts valid quantity", () => {
  const item = { status: "available", stock: 2 };

  assert.equal(isRentalAvailable(item, 3), false);
  assert.equal(isRentalAvailable(item, 2), true);
});
