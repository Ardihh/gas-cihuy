...
const rentalStatusSet = new Set(RENTAL_STATUSES);
...
function requireRentalStatus(record, field, ErrorType) {
  const value = requireText(record, field, ErrorType);

  if (!rentalStatusSet.has(value)) {
    throw new ErrorType("must be a supported rental status.", field);
  }
...
export function validateRentalInput(input, { today = getCurrentCalendarDate() } = {}) {
  if (!isRecord(input)) {
  ...
  if (startDate < today) {
    throw new RentalInputError("must be today or a future date.", "startDate");
  }
...
export function isRentalAvailable(item, quantity) {
...
export function normalizeRentalList(payload) {
  if (!Array.isArray(payload)) {
    throw new RentalResponseError("must be an array.");
  }
  ...
  return payload.map((record) => {
    ...
    const startDate = requireDate(record, "start_date", RentalResponseError);
    const endDate = requireDate(record, "end_date", RentalResponseError);
    ...
    return {
      id: requirePositiveInteger(record, "id", RentalResponseError),
      userId: requirePositiveInteger(record, "user_id", RentalResponseError),
      itemId: requirePositiveInteger(record, "item_id", RentalResponseError),
      itemName: requireText(record, "item_name", RentalResponseError),
      itemCategory: requireText(record, "item_category", RentalResponseError),
      startDate,
      endDate,
      quantity: requirePositiveInteger(record, "quantity", RentalResponseError),
      totalPrice: requireNonNegativeNumber(record, "total_price", RentalResponseError),
      status: requireRentalStatus(record, "status", RentalResponseError),
      adminNote: requireAdminNote(record),
    };
  });
}

export function filterRentalsByUser(rentals, userId) {
