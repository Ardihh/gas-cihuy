...
export async function registerUser(input) {
  try {
    return await registerUserWith(apiFetch, input);
  } catch (error) {
    if (error instanceof RegistrationServiceError) {
      if (error.code === "INVALID_INPUT") {
        throw new AuthServiceError("Data pendaftaran belum valid.", {
      ...
      throw new AuthServiceError("Layanan pendaftaran sedang tidak tersedia.", {
        code: "AUTH_SERVICE_UNAVAILABLE",
        status: error.status,
        cause: error,
      });
