...
import { adaptItem, unwrapCatalogResponse } from "./catalog-adapter.mjs";
import { formatRupiah } from "./format-currency.mjs";
...
test("unwrapCatalogResponse supports the verified catalog list envelope", () => {
  assert.deepEqual(
    unwrapCatalogResponse({ success: true, data: [validApiItem] }),
    [validApiItem],
  );
});

test("unwrapCatalogResponse supports the verified catalog detail envelope", () => {
  assert.deepEqual(
    unwrapCatalogResponse({ success: true, data: validApiItem }),
    validApiItem,
  );
...
test("unwrapCatalogResponse preserves direct catalog payload compatibility", () => {
  assert.equal(unwrapCatalogResponse(validApiItem), validApiItem);
  assert.deepEqual(unwrapCatalogResponse([validApiItem]), [validApiItem]);
});

test("unwrapCatalogResponse rejects an unsuccessful API envelope", () => {
  assert.throws(
    () => unwrapCatalogResponse({ success: false, data: [] }),
    /success/,
  );
});
...
test("formatRupiah formats numeric daily prices for presentation", () => {
  assert.equal(formatRupiah(75000), "Rp75.000");
});
