...
export function findReviewForRental(reviews, rentalId) {
  if (!Array.isArray(reviews) || !Number.isSafeInteger(rentalId) || rentalId <= 0) {
    return null;
  }

  return reviews.find((review) => review?.rentalId === rentalId) ?? null;
}
...
export function getReviewPresentation(rental, reviews, reviewState = "ready") {
  ...
  const review = findReviewForRental(reviews, rental.id);
  return review
    ? { status: "persisted", review }
