...
test("matches a persisted review by rental id and never by null rental id", () => {
  const reviews = [
    { id: 501, rentalId: 101, userId: 7, itemId: 41, rating: 5, comment: "Pertama." },
    { id: 504, rentalId: null, userId: 7, itemId: 41, rating: 5, comment: "Tanpa rental." },
  ];

  assert.equal(findReviewForRental(reviews, 101).id, 501);
  assert.equal(findReviewForRental(reviews, 41), null);
  assert.equal(findReviewForRental(reviews, 999), null);
});
